...
const total = myNumbers.reduce((accumulator, element, index, arr) => {
...
let myNumbersIndex = [] 
myNumbersIndex = myNumbers.forEach((element, index, arr) => {
	return index;
})
...
const enemies = people.filter(function(name) {
...
const totallyAwesome = people.reduce((accumulator, element, index, arr) => {
	return accumulator + element.awesomeLevel;
}, 0);